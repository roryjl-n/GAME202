	...
	while (!done) {
		...
		while (SDL_PollEvent(&event)) {
			...
			case SDL_QUIT:
				...
				break; 
				//when a key is pressed it automatically quits.
			case SDL_KEYDOWN: 
				done = 1;
				break;
				//outputs the (x, y) coordinates of the mouse when moving overtop the window
				//sand the coordinates where the cursor stops.
			case SDL_MOUSEMOTION:
				printf("Mouse moved by %d,%d to (%d,%d)\n",
					event.motion.xrel, event.motion.yrel,
					event.motion.x, event.motion.y);
				break;
				//outputs the button of the mouse pressed and the coordinates when pressed.
				//also outputs when a mouse button is pressed.
